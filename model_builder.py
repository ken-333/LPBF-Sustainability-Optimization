...
def create_lpbf_model(lt_val_um): # 输入参数是（layer thickness）lt_val_um: 层厚 (单位: um)，例如 80, 100, 120
 ...
 m.p = Var(bounds=cfg.BOUND['P'],domain=Reals,name="Power_W")  #Var(...) 是 Pyomo 创建决策变量的函数/类。
 m.v = Var(bounds=cfg.BOUND['V'],domain=Reals,name="Speed_mm_s")
 m.H = Var(bounds=cfg.BOUND['E'],domain=Reals,name="Hatch_um") 

 # ==========================
 # 3.Define auxiliary variable
 # ==========================
 ...
 m.BuildRate = Var(domain=NonNegativeReals, name="Inverse_BuildRate")
 ...
 m.obj_list.add(expr=-1 * expr_cost, sense=maximize)
